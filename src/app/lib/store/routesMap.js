...
export default {
                ...
                .then(function(player) {
                    ...
                    if (player.flags.noAliases === false) {
                        setMeta({
                            ...
                            description: `${
                                player.name
                            } player profile for Rainbow Six: Siege`,
                            type: "profile",
                        });
                    } else {
                        setMeta({
                            ...
                            description: `${
                                id
                            } account details in the community database for Rainbow Six: Siege`,
                        });
                    }
                })
                .catch(error => {
    ...
    COMPARISON: {
        ...
        thunk: async (dispatch, getState) => {
            ...
            api.getPlayers(idList).then(players => {
                dispatch({
                    type: "PLAYERS_FETCHED",
                    payload: players.map(p => ({ id: p.id, player: p })),
                });
            });
        },
    },
