...
export default function(player) {
    ...
    player.flags = {
        ...
        noPlaytime:
            !player.lastPlayed ||
            (!player.lastPlayed.casual && !player.lastPlayed.ranked),
    };
    ...
    const allRanks = player.seasonRanks
        ...
        .filter(x => !!x)
        .reduce((acc, rank) => {
            let alreadyHasSeason = !!acc.filter(x => x.season === rank.season)
                .length;
            if (!alreadyHasSeason) {
                acc.push(rank);
            }
    ...
    player.pastRanks = allRanks
        .map(x => {
            return [x.ncsa, x.emea, x.apac]
                ...
                .sort(
                    (a, b) =>
                        b.rank != a.rank ? b.rank - a.rank : b.mmr - a.mmr,
                )[0];
        })
