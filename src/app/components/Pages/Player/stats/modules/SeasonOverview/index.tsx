...
export default function SeasonOverview(props) {
    ...
    const ps =
        props.rank && props.rank.season
            ? pr.filter(x => x.season === props.rank.season)
            : pr;
    return (
        <div className="playermodule seasonoverview">
            {pr.map(rank => (
                <div
                    className={`pastrank season-${rank.season}`}
                    key={rank.season}
                >
                    <Icon
                        className="pastrank__icon"
                        glyph={GLYPHS["RANK" + rank.max_rank]}
                    />
                    <div className="pastrank__text">
                        <div className="pastrank__season">
                            {SEASONS[rank.season].name}
                        </div>
